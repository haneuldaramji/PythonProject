...
def update_status():  #가운데 위치한 상태표시문장(ex 총 일정: 4개 | 오늘 일정: 0개 | 오늘 날짜: 2026-05-17) 한줄을 매 입력마다 새로고침하는 함수
    ...
    for s in schedules: #전체일정리스트schedules에서 일정 하나씩 꺼냄
        if s[1] == today: #만약 전체일정에서 꺼낸 날짜가 현시점오늘날짜와 같다면
            today_count += 1 #오늘일정 개수 1 증가

    text = f"총 일정: {len(schedules)}개 | 오늘 일정: {today_count}개 | 오늘 날짜: {today}" #화면에 표시되는 문장

    status_label['text'] = text #상태표시라벨의 기존글자를 위에서 만든 text로 최신화


def show_rows(rows, empty_msg, dday_mode):  #rows 리스트 속 일정들을 결과창에 표시하는 함수
    ...
    if len(rows) == 0:  #만약 rows 리스트 속 일정이 0개면
        ...
        for s in rows: #rows 리스트 속 일정을 하나씩 꺼내서 반복
...
def show_all(): #전체일정리스트를 화면에 출력하는 함수
    rows = [] #화면에 출력할 리스트를 저장하는 빈 리스트

    for s in schedules: #전체일정리스트에서 일정을 하나씩 꺼냄
        rows.append(s) #전체일정리스트schedules에서 하나씩 꺼낸 리스트를 rows리스트에 추가

    rows.sort(key=sort_key_by_date_priority) #sort_key_by_date_priority 기준 함수를 사용하여 리스트 정렬

    show_rows(rows, "등록된 일정이 없습니다.", False) #부합하는 일정들을 결과창에 출력
...
def open_date_input_window(title, guide_text, button_text, dday_mode):  # 날짜별조회 입력용/D-day계산입력용 보조창을 여는 함수
    ...
    def run(): # 보조창 버튼을 눌렀을 때 실제로 실행되는 내부함수
        ...
        rows = [] # 조건을 만족하는 일정을 저장할 빈 리스트 rows 설정

        for s in schedules: # 전체일정리스트에서 일정 한개씩 추출
            if s[1] == target_date: # 만약 꺼낸일정의 날짜가 입력한 날짜와 같다면
                rows.append(s) # 해당 일정을 rows 리스트에 추가

        rows.sort(key=sort_key_by_date_priority) # 입력한 날짜들의 일정을 우선순위 기준으로 정렬

        show_rows(rows, "해당 날짜의 일정이 없습니다.", dday_mode) # 부합하는 일정들을 결과창에 출력 (dday_mode에 따라 다르게 표시)
...
def sort_by_priority(): #전체일정리스트를 우선순위순으로 화면에 출력하는 함수
    ...
    rows.sort(key=sort_key_by_priority_date) #sort_key_by_priority_date 기준 함수를 사용하여 리스트 정렬

    show_rows(rows, "등록된 일정이 없습니다.", False) #show_rows함수 통해 정렬된 전체일정을 일반출력모드로 결과창에 출력
...
def delete_selected(): #결과창에서 선택한 일정을 삭제하는 함수
    ...
    target = current_rows[index] #현재 화면일정리스트에서 선택한 번호에 해당하는 일정을 삭제대상으로 지정

    if target in schedules: #만약 삭제대상 일정이 전체일정리스트 안에 있다면
        schedules.remove(target) #전체일정리스트schedules에서 삭제대상 일정을 제거
        save_schedules() #save_schedules함수통해 변경된 전체일정리스트를 파일에 저장
        show_all() #삭제 후 show_all함수 통해 전체일정을 다시 출력
        messagebox.showinfo("삭제 완료", "선택한 일정이 삭제되었습니다.") #삭제완료 메시지창띄움
    else: #만약 삭제대상 일정을 전체일정리스트에서 찾지 못했다면
        messagebox.showerror("삭제 오류", "삭제할 일정을 찾지 못했습니다.") #삭제실패 메시지창띄움
...
root.geometry("760x560") #메인창 크기 가로760 세로560으로 설정
